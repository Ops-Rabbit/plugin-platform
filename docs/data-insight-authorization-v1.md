...
Available in `@opsrabbit/plugin-sdk` 0.20.0. Plugin-native query and governed-action mappings are available in 0.21.0.
...
In SDK 0.21, a dashboard template query may use `plugin_query.saved_query_id` when the manifest declares `dataInsight.pluginQueryAction` as a viewer read action. A governed template must list that id under `authorization.references.saved_queries`. A read-only Forms action may declare `dataInsightAuthorization.namespace` and `inputField`; the host authorizes that top-level scalar input before invoking the plugin. These declarations never replace the plugin's own catalog and attestation checks.
