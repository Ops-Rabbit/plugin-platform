...
3. Bootstrap version `0.1.0` using an npm granular access token stored only as the
   environment secret `NPM_TOKEN`. Never commit or paste the token into an issue.
